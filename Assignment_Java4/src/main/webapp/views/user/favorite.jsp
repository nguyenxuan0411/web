	...
	<main style="display: flex; min-height: 100vh; flex-direction: column;">
		<section class="section">
			<div class="container mt-6">
				<div class="notification is-info content" ${empty message ? 'hidden' : message}>
					<h5 class="has-text-white has-text-centered">${message}</h5>
				</div>
				<c:if test="${empty videos}">
					<div class="notification is-danger content">
						<h5 class="has-text-white has-text-centered">You don't have any favorite videos!</h5>
					</div>
				</c:if>
					...
					<c:forEach var="item" items="${videos}">
						<div class="column is-6-mobile is-3-desktop">
							...
							<div class="buttons is-fullwidth">
								<a href="UnlikeVideo?videoId=${item.videoId}" class="button is-info"> <span class="icon"> <i
										class="far fa-thumbs-down"></i>
								</span> <span>UNLIKE</span>
